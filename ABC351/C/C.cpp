...
int main() {
    ...
    while(flag){
        vec_ans = {vec[0]};
        rep1(i, N){
            if(vec_ans[vec_ans.size()-1] == vec[i]) vec_ans[vec_ans.size()-1]++;
            else vec_ans.push_back(vec[i]);

            // cout << i << ": ";
            // rep(i, vec_ans.size()) cout << vec_ans[i] << " ";
            // cout << endl;
        }
        if(vec.size() == vec_ans.size()) break;
        vec = vec_ans;
        vec_ans.clear();
    }
    cout << "ans " << vec_ans.size() << endl;
    //rep(i, vec_ans.size()) cout << vec_ans[i] << endl;
    return 0;
}
