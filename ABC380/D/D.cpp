...
int main() {
    ...
    ll ktmr_n;
    rep(i, Q){
        ktmr_n = Kvec[i] / S.size();
        bitset<110> bs(ktmr_n);
        ll cnt_bit_1 = 0;
        rep(bit_idx, 110){
            if(bs[bit_idx]) cnt_bit_1++;
        }

        // 該当するインデックス
        ll idx = Kvec[i] % S.size();
        // cout <<"@: "<< Kvec[i] <<"/" << S.size() << "-> " << idx << endl;
        ...
        if(cnt_bit_1 % 2 == 0){
            ...
            // cout << "non\n";
            // 小文字なら大文字に
            if(islower(S[idx])) tmp = toupper(S[idx]);
            // 大文字なら小文字に
            else tmp = tolower(S[idx]);
