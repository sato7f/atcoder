...
int main() {
    ...
    // cout << N << ", " << flip_idx << ", " << S << endl;;
    vp vec1;
    int st1=-1,ed1=-1;

    // １のかたまりの始まりと終わりの添え字を記録
    for(ll i = 0; i < S.size(); i++){
        // cout << "i:" << i << endl;
        if(st1 == -1 && S[i] == '1') st1=i;
        if( (st1 != -1 && S[i] == '0')|| (st1 != -1 && i==N-1)){
            if(st1 != -1 && i==N-1){
                ed1=N;
                // cout << "ok" << endl;
            }
            else ed1=i;
            vec1.push_back({st1,ed1});
            // cout << st1 << ", " << ed1 << endl;
            st1 = -1;
            ed1 = -1;
        }
    ...
    // 所望の番目の１のかたまりの添え字を
    ll flip_wise = vec1[flip_idx-1].second - vec1[flip_idx-1].first;
    // cout << "fw: " << flip_wise << endl;

    for(ll i = vec1[flip_idx-1].first; i < vec1[flip_idx-1].first + flip_wise; i++){
        S[i] = '0';
    }

    for(ll i = vec1[flip_idx-1-1].second; i < vec1[flip_idx-1-1].second + flip_wise; i++){
        S[i] = '1';
    }
