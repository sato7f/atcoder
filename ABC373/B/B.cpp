...
int main() {
    ll sum_step = 0;
    string key_map;
    string key_kotei = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    cin >> key_map;


    ll new_idx, old_idx = key_map.find(key_kotei[0]);
    ll tmp = old_idx;
    
    for(ll i = 1; i < key_kotei.size(); i++){
        // 次のインデックスを見る
        new_idx = key_map.find(key_kotei[i]);
        // cout << "old: " << old_idx << ", new: " << new_idx << endl;
        sum_step += abs(new_idx - old_idx);
        // cout << i << ", " << sum_step << "\n" << endl;
