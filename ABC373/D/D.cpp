...
void dfs(vector<vp> adj, ll v) {
    // cout << v << endl;

    seen[v] = true; // v を訪問済にする

    // v から行ける各頂点 next_v について
    for (auto next_v_w : adj[v]) {
        auto next_v = next_v_w.first;
        if (seen[next_v]) continue; // next_v が探索済だったらスルー
        
        // 処理
        ans[next_v] = next_v_w.second;

        dfs(adj, next_v); // 再帰的に探索
    }
}
...
int main() {
    ...
    rep(i, M){
        ...
        // cout << "ok! " << edg[i][0] << endl;
        adj[edg[i][0] - 1].push_back({edg[i][1] - 1, edg[i][2]});
    }
